...
namespace TechnicalTestScaffoldingUnitTests
{
    ...
    public class UnitTest1
    {
        ...
        public class ScoreTests
        {
            ...
            {
                Assert.AreEqual(6, CardsTest.CalculateScore(new int[] { 2, 3, 3, 3, 10 }).score);
                Assert.AreEqual(6, CardsTest.CalculateScore(new int[] { 3, 2, 3, 3, 10 }).score);
                Assert.AreEqual(6, CardsTest.CalculateScore(new int[] { 3, 3, 2, 3, 10 }).score);
                Assert.AreEqual(6, CardsTest.CalculateScore(new int[] { 3, 3, 3, 2, 10 }).score);
                Assert.AreEqual(6, CardsTest.CalculateScore(new int[] { 10, 3, 3, 2, 3 }).score);
            }
